...
import random
pygame.init()
pygame.display.set_caption('A moving ball')
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type==pygame.MOUSEBUTTONDOWN:
            ...
            if(p>exitrect.left and p<exitrect.right and q>exitrect.top and q<exitrect.bottom):
                print("helloworld\n")
                running= False
    ...
    t=(m**2+n**2)**0.5
    if touch_edge==False:
        if(length<=111 and iscover==False):
            speed[1] = n/t*5
            speed[0] = m/t*5
            iscover=True
            sc=sc+0.001
        if(length>111 and iscover==True):
            iscover=False
